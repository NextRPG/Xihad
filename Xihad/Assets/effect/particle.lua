...
local function makeIce(pnode, f, size, radius, pps)
	local width = size
	...
	local xi = f:initer("Cylinder")
	xi:setLength(0)
	xi:setRadius(radius)
	xi:setNormalDirectionSize(0)
	xi:setOutlineOnly(false)
...
---------------------------------------------- 	
	env:deferMessage(0, "attack begin")
	env:deferMessage(1.3, "attack end")
end
