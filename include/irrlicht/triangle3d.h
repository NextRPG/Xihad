...
namespace irr
{
	...
	public:
			...
			const vector3d<T>& lineVect, vector3d<T>& outIntersection) const
		{
			...
			vector3d<irr::f64> normalf64 = trianglef64.getNormal();
			normalf64.normalize();
			f64 t2;
		...
		{
			return (pointB-pointA).crossProduct(pointC-pointA);//normal.crossProduct(buf);;
		}
		...
		{
			const f32 d = (f32)getNormal().dotProduct(lookDirection);
		...
		T getArea() const
		{
			return getNormal().getLength() * 0.5f;
