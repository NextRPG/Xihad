...
{
	struct InheritenceTree::impl
	{
		vector<InheritenceChain::Node>	hierarchy;
		unordered_map<string, int>  type2Index;
	};

	InheritenceTree::InheritenceTree() : mImpl(new impl)
	{
	}
	...
	InheritenceTree::RegisterResult InheritenceTree::registerType( 
	...
	bool InheritenceTree::containsType( const std::string& className ) const
	{
		return mImpl->type2Index.find(className) != mImpl->type2Index.end();
	}

	InheritenceChain InheritenceTree::getInheritenceChain( const std::string& className ) const
	{
		auto got = mImpl->type2Index.find(className);

		if (got != mImpl->type2Index.end())
			return InheritenceChain(got->second, mImpl->hierarchy);
		else
			return InheritenceChain();
	}

	InheritenceTree::Iterator InheritenceTree::begin() const
	{
		return mImpl->hierarchy.begin();
	}

	InheritenceTree::Iterator InheritenceTree::end() const
	{
