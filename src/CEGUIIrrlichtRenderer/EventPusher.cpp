...
namespace CEGUI
{
	IrrlichtEventPusher::IrrlichtEventPusher( irr::gui::ICursorControl* ctrl ) : 
		d_cursorctrl(ctrl)
	{
		initCodes();
	};

	bool IrrlichtEventPusher::OnEvent( const SEvent& event )
	{
	...
	bool IrrlichtEventPusher::OnKeyDown( EKEY_CODE key, wchar_t wch, bool /*ctrl*/, bool /*shift*/ )
	{
		bool handled = false;
	...
	bool IrrlichtEventPusher::OnKeyUp( EKEY_CODE key, wchar_t /*wch*/, bool /*ctrl*/, bool /*shift*/ )
	{
		bool handled = false;
		CEGUI::GUIContext& cegui = CEGUI::System::getSingleton().getDefaultGUIContext();
	...
	bool IrrlichtEventPusher::OnMouse( s32 x, s32 y, f32 w, EMOUSE_INPUT_EVENT e )
	{
		using namespace irr;
		bool handled = false;
	...
	void IrrlichtEventPusher::initCodes()
	{
		using namespace irr;
		...
		irr2ceCODE[KEY_LWIN      ] = Key::LeftWindows;  // Left Windows key
	...
	CEGUI::Key::Scan IrrlichtEventPusher::getKeyCode( irr::EKEY_CODE kc ) const
	{
